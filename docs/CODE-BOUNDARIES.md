...
Package imports use `trinetra`; `package.json` is the authoritative
...
`trinetra/build/vite` is a separate Node-only export. `build/vite.js`
...
```js
import { createBrowserViteConfig } from 'trinetra/build/vite';
...
The portable `trinetra/sources/adsb-lol` export normalizes existing aircraft
...
`trinetra/server/providers/places` exports the Google nearby-place and
...
`trinetra/sources/places` exports portable response projections for Google
...
`trinetra/server/providers/space` is a Node-only entry for the CelesTrak
...
`trinetra/sources/space` owns only fixed upstream URL construction: the
...
`trinetra/server/providers/terrain`, `/traffic`, `/firms` and `/gbfs`
...
`trinetra/sources/terrain` exports existing point-key, retry and cache
reconstruction mechanics with injectable acquisition dependencies.
`trinetra/sources/traffic` exports tile math and budget calculations.
`trinetra/sources/gbfs` exports host/path acceptance and cache-header rules.
...
`trinetra/sources/firms-csv` exports the existing CSV parser, header
...
`trinetra/search` exports an explicit geocoding service and Google/Photon
...
`trinetra/ui/panels` owns collapse-button binding, nearest-panel Escape
...
`trinetra/ui/surfaces` exports `createSurfaceKeyboard` from
...
`trinetra/ui/layout` exports synchronous `layoutLeftPanelRail` and
...
`trinetra/ui/input` exports `bindApplicationShortcuts` and
...
`trinetra/scenes` exports `SceneDirector`. Its `subscribe(listener)` supplies
...
`trinetra/application/catalog` captures caller-supplied layer instances and
...
`trinetra/sources/reference` constructs fresh earthquake and bundled cable
