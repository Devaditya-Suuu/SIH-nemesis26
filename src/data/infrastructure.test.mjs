...
import { createLocalGeoJsonLayer } from 'trinetra/infrastructure/geojson';
...
test('package exports import without an application, DOM, fetch, or timers', () => {
  const result = spawnSync(
    ...
    await import('trinetra/infrastructure');
    await import('trinetra/infrastructure/geojson');
    await import('trinetra/infrastructure/lod');
  `,
    ],
...
test('consumer build includes only infrastructure code and resolves assets under a non-root base', async () => {
  ...
  const result = await build({
    ...
    build: {
      ...
      rollupOptions: {
        input: fileURLToPath(
          import.meta.resolve('trinetra/infrastructure'),
        ),
        external: ['cesium'],
